...
| Farm + potion income | 213,694.02 T | 219,423.8997 T |
| Full income | 302,701.64 T | 308,337.4171 T |
| Highest-income potion | 125,000 | 126,000 |
| Income at best potion | 236,499.9 T | 241,176.7724 T |
| Loss from current potion | 22,805.9 T | 21,752.8727 T |
| Percent loss | 9.643091518% | 9.019472515% |
...
`validation.test.js` runs the Hohmono snapshot through the same `calculator.js` engine used in the browser. It asserts the three 27,400 daily-action constants, the documented current TSer outputs and ROI values, the 126,000 best-potion result, and the shared Battler/TSer Tome Drop ROI. Run it with `node validation.test.js`; it requires no installed packages.
